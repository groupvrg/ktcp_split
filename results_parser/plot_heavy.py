...
pl.suptitle("#vms by project");

gcp = pl.bar(vms.index, vms['GCP'], color='red') 
aws = pl.bar(vms.index, vms['AWS'], color='orange',bottom=vms['GCP']) 
azure = pl.bar(vms.index, vms['AZURE'], color='blue') 
pl.xticks(vms.index, vms.proj, fontsize=fontsize, rotation=-60)

list_t = vms['total'].tolist()
for i in range(len(vms.index)):
	pl.text(x = i, y = list_t[i], s = list_t[i])

pl.legend([gcp,aws,azure], ['GCP','AWS','AZURE'])
pl.grid(axis='y', linestyle=':')
pl.savefig('heavy.pdf', format='pdf');
pl.show()
